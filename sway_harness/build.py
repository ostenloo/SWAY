    ...
    try:
        ...
        for iteration in range(start_iter, start_iter + build_cfg.max_iterations):
            ...
            if conv.converged:
                ...
                # SWAY_FORCE_FULL_ITERS: for a CONV-derivation bootstrap we want the
                # full iteration trajectory even after the (borrowed) bar certifies, so
                # the derived threshold isn't shaped by early-stop. Keep the certified
                # candidate as best but do NOT return early.
                if not os.environ.get("SWAY_FORCE_FULL_ITERS"):
